...
export default function HomePage() {
  ...
  return (
    <>
      <section className="relative text-white overflow-hidden" style={{background:'linear-gradient(135deg, var(--brand-dark) 0%, var(--brand) 50%, #7c3aedcc 100%)'}}>
        <div className="absolute inset-0 opacity-10" style={{backgroundImage:'radial-gradient(circle, white 1px, transparent 1px)', backgroundSize:'30px 30px'}}/>
        <div className="relative max-w-7xl mx-auto px-6 py-28 md:py-40 text-center">
          <div className="text-6xl mb-6">🎯</div>
          <div className="inline-block bg-white/20 backdrop-blur text-white text-xs font-bold tracking-widest uppercase px-5 py-2 rounded-full mb-8">Fresh Drop 🔥</div>
          <h1 className="text-6xl md:text-8xl font-black leading-none mb-6" style={{textShadow:'0 4px 20px rgba(0,0,0,0.3)'}}>FRESH DROPS</h1>
          <p className="text-white/80 text-xl mb-12 max-w-xl mx-auto">Limited edition pieces crafted for the bold.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/products" className="inline-block bg-white text-black px-10 py-4 text-base font-black rounded-full hover:scale-105 transition-transform shadow-lg">Shop Now →</Link>
            <Link href="/products" className="inline-block border-2 border-white/50 text-white px-10 py-4 text-base font-semibold rounded-full hover:bg-white/10 transition">Browse All 👀</Link>
          </div>
        </div>
      </section>
